...
//        });
    ...
    private void captureBankPaymentInfo() {
      ...
      bankName = bankNameEditText.getText().toString();
        ...
        if (TextUtils.isEmpty(bankAcctName) || TextUtils.isEmpty(bankAcctNo) || TextUtils.isEmpty(bankName)) {
        ...
        mRef.child("RedeemUSD").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists() && dataSnapshot.getValue() != null) {
                    try {
                   usermoney = Float.parseFloat(dataSnapshot.getValue().toString());
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists() && dataSnapshot.getValue() != null) {
                    try {
                        ...
                        usermoneyCoins = Integer.parseInt(dataSnapshot.getValue().toString());
...
//                usermoneyCoins = Integer.parseInt(dataSnapshot.getValue(String.class));
        ...
        mRef.child("Coins").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists() && dataSnapshot.getValue() != null) {
                    try {
                        usercoins = Integer.parseInt(dataSnapshot.getValue().toString());
            ...
            Thread sender = new Thread(() -> {
                try {
                   ...
                   bankName = bankNameEditText.getText().toString();
                    ...
                    int result = 0;
                    // Ensure usercoins is not negative after subtraction
                    if (usermoneyCoins >= 1) {
                        result = usercoins - usermoneyCoins;
                    } else {
                        // Handle the case where subtracting usermoneyCoins results in a negative value
                        // For example, you might set usercoins to 0 or take appropriate action
                        Toast.makeText(RedeemPayTm2.this, "Sorry,Try again", Toast.LENGTH_SHORT).show();
...
//                    coinsEdit.apply();
                    ...
                    // Ensure usercoins is not negative after subtraction
                    if (result >= 0) {
                        mRef.child("Coins").setValue(result);
                    } else {
                        // Handle the case where subtracting usermoneyCoins results in a negative value
                        // For example, you might set usercoins to 0 or take appropriate action
